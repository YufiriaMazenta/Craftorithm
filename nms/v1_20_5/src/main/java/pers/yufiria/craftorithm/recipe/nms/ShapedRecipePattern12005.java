...
public final class ShapedRecipePattern12005 extends CustomShapedRecipePattern<InventoryCrafting> {

    ShapedRecipePattern12005(int width, int height, List<Optional<RecipeChoice>> ingredients) {
    ...
    public boolean matches(InventoryCrafting craftingInput) {
        for (int i = 0; i <= craftingInput.f() - this.width; ++i) {
            for (int j = 0; j <= craftingInput.g() - this.height; ++j) {
                if (this.matches(craftingInput, i, j, true)) return true;
                if (this.matches(craftingInput, i, j, false)) return true;
            }
        }
        return false;
    }
    ...
    public static ShapedRecipePattern12005 fromBukkitRecipe(ShapedRecipe shapedRecipe) {
        ...
        return new ShapedRecipePattern12005(width, height, ingredients);
    }
}
