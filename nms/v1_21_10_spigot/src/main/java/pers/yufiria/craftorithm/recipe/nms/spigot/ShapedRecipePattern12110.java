...
public final class ShapedRecipePattern12110 extends CustomShapedRecipePattern<CraftingInput> {

    public ShapedRecipePattern12110(int width, int height, List<Optional<RecipeChoice>> ingredients) {
        super(width, height, ingredients);
    }

    public static ShapedRecipePattern12110 fromBukkitRecipe(ShapedRecipe shapedRecipe) {
        ...
        return new ShapedRecipePattern12110(width, height, ingredients);
    }

    public boolean matches(CraftingInput craftingInput) {
        if (craftingInput.e() != this.ingredientCount) return false;
        if (craftingInput.f() != width) return false;
        if (craftingInput.g() != height) return false;
