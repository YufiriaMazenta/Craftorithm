package pers.yufiria.craftorithm.ui.creator.smithing;
...
import pers.yufiria.craftorithm.config.menu.creator.VanillaSmithingCreatorConfig;
...
public class VanillaSmithingCreator extends RecipeCreator {
    ...
    public VanillaSmithingCreator(@NotNull Player player, @Nullable String recipeName) {
        super(player, recipeName);
        this.display = new MenuDisplay(
            VanillaSmithingCreatorConfig.TITLE.value(),
    ...
    protected Icon getFrameIcon() {
        return CreatorIconParser.INSTANCE.parse(VanillaSmithingCreatorConfig.FRAME_ICON.value()).get();
    }

    @Override
    protected Icon getResultFrameIcon() {
        return CreatorIconParser.INSTANCE.parse(VanillaSmithingCreatorConfig.RESULT_FRAME_ICON.value()).get();
    }

    private Icon getConfirmIcon() {
        IconDisplay iconDisplay = CreatorIconParser.INSTANCE.parseIconDisplay(
            VanillaSmithingCreatorConfig.CONFIRM_ICON.value()
        );
            ...
            public Icon onClick(InventoryClickEvent event) {
                ...
                if (loadResult) {
                    LangUtils.sendLang(
                        ...
                        Map.of(
                            "<recipe_type>",
                            Languages.RECIPE_TYPE_NAME_VANILLA_SMITHING.value((Player) event.getWhoClicked()),
                            "<recipe_name>",
                            recipeName
                        )
                    );
