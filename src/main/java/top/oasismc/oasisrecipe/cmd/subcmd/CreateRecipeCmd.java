...
import top.oasismc.oasisrecipe.menu.RecipeMenuBuilder;
import top.oasismc.oasisrecipe.recipe.RecipeType;
...
public class CreateRecipeCmd extends AbstractSubCommand {
    ...
    private CreateRecipeCmd() {
        super("create");
        recipeTypeList = Arrays.stream(RecipeType.values()).map(type -> type.name().toLowerCase(Locale.ROOT)).collect(Collectors.toList());
    }
    ...
    public boolean onCommand(CommandSender sender, List<String> args) {
        ...
        ((Player) sender).openInventory(RecipeMenuBuilder.buildMenu(args.get(0), args.get(1)));
        return true;
    }
